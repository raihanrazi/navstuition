<?php
/**
 * Media Grabber Script - A script for grabbing media related to a post.
 * Hybrid Core Framework, Justin Tadlock http://themehybrid.com/
 *
 * Media Grabber is a script for pulling media either from the post content or attached to the 
...
/**
 * Wrapper function for the ChromaticFw_Media_Grabber class.  Returns the HTML output for the found media.
 *
 * @since 1.0.0
 * @access public
 * @param array
 * @return string
 */
function chromaticfw_media_grabber( $args = array() ) {

	$media = new ChromaticFw_Media_Grabber( $args );
 ...
 * @return void
 */
class ChromaticFw_Media_Grabber {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'chromaticfw_media_grabber_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'chromaticfw_media_grabber_autoembed_media',       array( $wp_embed, 'autoembed' ) );
		...
		$this->args    = apply_filters( 'chromaticfw_media_grabber_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'chromaticfw_media_grabber_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'chromaticfw_media_grabber_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'chromaticfw_media_grabber_autoembed_media', $value[0] ) );
	...
	public function filter_dimensions( $html ) {
		...
		list( $width, $height ) = apply_filters( 
			'chromaticfw_media_grabber_dimensions', 
