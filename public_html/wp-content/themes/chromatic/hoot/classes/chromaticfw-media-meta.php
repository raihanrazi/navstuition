...
class ChromaticFw_Media_Meta {
	...
	public function image_meta() {
		...
		if ( !empty( $this->meta['width'] ) && !empty( $this->meta['height'] ) ) {
			...
			$this->items['dimensions'] = array(
				...
				'<a href="' . esc_url( wp_get_attachment_url() ) . '">' . sprintf( __( '%1$s &#215; %2$s', 'chromaticfw' ), number_format_i18n( absint( $this->meta['width'] ) ), number_format_i18n( absint( $this->meta['height'] ) ) ) . '</a>',
				__( 'Dimensions', 'chromaticfw' )
			);
		}

		/* If a timestamp exists, add it to the $items array. */
		if ( !empty( $this->meta['image_meta']['created_timestamp'] ) )
			$this->items['created_timestamp'] = array( date_i18n( get_option( 'date_format' ), strip_tags( $this->meta['image_meta']['created_timestamp'] ) ), __( 'Date', 'chromaticfw' ) );

		/* If a camera exists, add it to the $items array. */
		if ( !empty( $this->meta['image_meta']['camera'] ) )
			$this->items['camera'] = array( esc_html( $this->meta['image_meta']['camera'] ), __( 'Camera', 'chromaticfw' ) );

		/* If an aperture exists, add it to the $items array. */
		if ( !empty( $this->meta['image_meta']['aperture'] ) )
			$this->items['aperture'] = array( sprintf( '<sup>f</sup>&#8260;<sub>%s</sub>', absint( $this->meta['image_meta']['aperture'] ) ), __( 'Aperture', 'chromaticfw' ) );
		...
		if ( !empty( $this->meta['image_meta']['focal_length'] ) )
			...
			$this->items['focal_length'] = array( sprintf( __( '%s mm', 'chromaticfw' ), absint( $this->meta['image_meta']['focal_length'] ) ), __( 'Focal Length', 'chromaticfw' ) );
		...
		if ( !empty( $this->meta['image_meta']['iso'] ) ) {
			$this->items['iso'] = array(
				...
				'<abbr title="' . __( 'International Organization for Standardization', 'chromaticfw' ) . '">' . __( 'ISO', 'chromaticfw' ) . '</abbr>'
			);
		}
		...
		if ( !empty( $this->meta['image_meta']['shutter_speed'] ) ) {
			...
			$this->items['shutter_speed'] = array( sprintf( __( '%s sec', 'chromaticfw' ), $shutter_speed ), __( 'Shutter Speed', 'chromaticfw' ) );
		}
	}
	...
	public function audio_meta() {
		...
		$this->items['file_name'] = array( '<a href="' . esc_url( wp_get_attachment_url( $this->args['post_id'] ) ) . '">' . basename( get_attached_file( $this->args['post_id'] ) ) . '</a>', __( 'File Name', 'chromaticfw' ) );
		...
		if ( preg_match( '/^.*?\.(\w+)$/', get_attached_file( $this->args['post_id'] ), $matches ) )
			$this->items['file_type'] = array( esc_html( strtoupper( $matches[1] ) ), __( 'File Type', 'chromaticfw' ) );
	...
	public function video_meta() {
		...
		if ( !empty( $this->meta['width'] ) && !empty( $this->meta['height'] ) )
			/* Translators: Media dimensions - 1 is width and 2 is height. */
			$this->items['dimensions'] = array( sprintf( __( '%1$s &#215; %2$s', 'chromaticfw' ), number_format_i18n( absint( $this->meta['width'] ) ), number_format_i18n( absint( $this->meta['height'] ) ) ), __( 'Dimensions', 'chromaticfw' ) );
