...
add_filter( 'body_class', 'chromaticfw_body_class_filter', 0, 2 );

/* Filters the WordPress 'post_class' early. */
add_filter( 'post_class', 'chromaticfw_post_class_filter', 0, 3 );

/* Filters the WordPress 'comment_class' early. */
add_filter( 'comment_class', 'chromaticfw_comment_class_filter', 0, 3 );
 ...
 * @return array
 */
function chromaticfw_get_context() {
	global $chromaticfw;

	/* If $chromaticfw->context has been set, don't run through the conditionals again. Just return the variable. */
	if ( isset( $chromaticfw->context ) )
		return apply_filters( 'chromaticfw_context', $chromaticfw->context );

	/* Set some variables for use within the function. */
	$chromaticfw->context = array();
	$object = get_queried_object();
	$object_id = get_queried_object_id();

	/* Front page of the site. */
	if ( is_front_page() )
		$chromaticfw->context[] = 'home';

	/* Blog page. */
	if ( is_home() ) {
		$chromaticfw->context[] = 'blog';
	}

	/* Singular views. */
	elseif ( is_singular() ) {
		$chromaticfw->context[] = 'singular';
		$chromaticfw->context[] = "singular-{$object->post_type}";
		$chromaticfw->context[] = "singular-{$object->post_type}-{$object_id}";
	}

	/* Archive views. */
	elseif ( is_archive() ) {
		$chromaticfw->context[] = 'archive';

		/* Post type archives. */
		if ( is_post_type_archive() ) {
			$post_type = get_post_type_object( get_query_var( 'post_type' ) );
			$chromaticfw->context[] = "archive-{$post_type->name}";
		}

		/* Taxonomy archives. */
		if ( is_tax() || is_category() || is_tag() ) {
			$chromaticfw->context[] = 'taxonomy';
			$chromaticfw->context[] = "taxonomy-{$object->taxonomy}";

			$slug = ( ( 'post_format' == $object->taxonomy ) ? str_replace( 'post-format-', '', $object->slug ) : $object->slug );
			$chromaticfw->context[] = "taxonomy-{$object->taxonomy}-" . sanitize_html_class( $slug, $object->term_id );
		}

		/* User/author archives. */
		if ( is_author() ) {
			$user_id = get_query_var( 'author' );
			$chromaticfw->context[] = 'user';
			$chromaticfw->context[] = 'user-' . sanitize_html_class( get_the_author_meta( 'user_nicename', $user_id ), $user_id );
		}

		/* Date archives. */
		if ( is_date() ) {
			$chromaticfw->context[] = 'date';

			if ( is_year() )
				$chromaticfw->context[] = 'year';

			if ( is_month() )
				$chromaticfw->context[] = 'month';

			if ( get_query_var( 'w' ) )
				$chromaticfw->context[] = 'week';

			if ( is_day() )
				$chromaticfw->context[] = 'day';
		}

		/* Time archives. */
		if ( is_time() ) {
			$chromaticfw->context[] = 'time';

			if ( get_query_var( 'hour' ) )
				$chromaticfw->context[] = 'hour';

			if ( get_query_var( 'minute' ) )
				$chromaticfw->context[] = 'minute';
		}
	}

	/* Search results. */
	elseif ( is_search() ) {
		$chromaticfw->context[] = 'search';
	}

	/* Error 404 pages. */
	elseif ( chromaticfw_is_404() ) {
		$chromaticfw->context[] = 'error-404';
	}

	return array_map( 'esc_attr', apply_filters( 'chromaticfw_context', array_unique( $chromaticfw->context ) ) );
}
 ...
 * @return array
 */
function chromaticfw_body_class_filter( $classes, $class ) {
	...
	/* Locale and language. */
	$locale = get_locale();
	$lang   = chromaticfw_get_language( $locale );
	...
	$classes = array_merge( $classes, chromaticfw_get_context() );
 ...
 * @return array
 */
function chromaticfw_post_class_filter( $classes, $class, $post_id ) {
 ...
 * @return array
 */
function chromaticfw_comment_class_filter( $classes, $class, $comment_id ) {
