 ...
 * @return void
 */
function chromaticfw_set_prefix( $prefix ) {
	global $chromaticfw;

	$chromaticfw->prefix = sanitize_key( apply_filters( 'chromaticfw_prefix', $prefix ) );
}

/**
 * Defines the theme prefix. This allows developers to infinitely change the theme. In theory,
 * one could use the Framework core to create their own theme or filter 'chromaticfw_prefix' with a 
 ...
 * @access public
 * @global object $chromaticfw
 * @return string $chromaticfw->prefix The prefix of the theme.
 */
function chromaticfw_get_prefix() {
	global $chromaticfw;

	/* If the global prefix isn't set, define it. Plugin/theme authors may also define a custom prefix. */
	if ( empty( $chromaticfw->prefix ) )
		$chromaticfw->prefix = sanitize_key( apply_filters( 'chromaticfw_prefix', get_template() ) );

	return $chromaticfw->prefix;
}
 ...
 *
 * An example of a basic hook would be 'chromaticfw_header'.  The do_atomic() function extends that to 
 * give extra hooks such as 'chromaticfw_singular_header', 'chromaticfw_singular-post_header', and 
 * 'chromaticfw_singular-post-ID_header'.
 *
 ...
 */
function chromaticfw_do_atomic( $tag = '', $arg = '' ) {
	...
	foreach ( (array) chromaticfw_get_context() as $context )
 ...
 * without having to know how to use WordPress conditional tags.  The theme handles the logic.
 *
 * An example of a basic hook would be 'chromaticfw_entry_meta'.  The apply_atomic() function extends 
 * that to give extra hooks such as 'chromaticfw_singular_entry_meta', 'chromaticfw_singular-post_entry_meta', 
 * and 'chromaticfw_singular-post-ID_entry_meta'.
 *
 * @since 1.0.0
 * @access public
 * @param string $tag     Usually the location of the hook but defines what the base hook is.
 ...
 */
function chromaticfw_apply_atomic( $tag = '', $value = '' ) {
...
/**
 * Wraps the output of chromaticfw_apply_atomic() in a call to do_shortcode(). This allows developers to use 
 ...
 * @return mixed  $value The value after it has been filtered.
 */
function chromaticfw_apply_atomic_shortcode( $tag = '', $value = '' ) {
	return do_shortcode( chromaticfw_apply_atomic( $tag, $value ) );
}
 ...
 */
function chromaticfw_format_hook( $tag, $context = '' ) {
	return chromaticfw_get_prefix() . ( ( !empty( $context ) ) ? "_{$context}" : "" ). "_{$tag}";
}
 ...
 */
function chromaticfw_set_content_width( $width = '' ) {
 ...
 */
function chromaticfw_get_content_width() {
	global $content_width;
