...
function chromaticfw_attachment_id3_keys( $fields, $attachment, $context ) {
	...
	if ( 'display' === $context ) {
		...
		$fields['filesize']		 = __( 'File Size', 'chromaticfw' );
		$fields['mime_type']		= __( 'Mime Type', 'chromaticfw' );
		$fields['length_formatted'] = __( 'Run Time',  'chromaticfw' );
	}

	if ( chromaticfw_attachment_is_audio( $attachment->ID ) ) {

		$fields['genre']		= __( 'Genre',	'chromaticfw' );
		$fields['year']		 = __( 'Year',	 'chromaticfw' );
		$fields['composer']	 = __( 'Composer', 'chromaticfw' );
		$fields['track_number'] = __( 'Track',	'chromaticfw' );

		if ( 'display' === $context )
			$fields['unsynchronised_lyric'] = __( 'Lyrics', 'chromaticfw' );
	}
