 ...
 * 'chromaticfw-core-scripts' feature to allow the framework to handle loading the stylesheets into 
...
add_action( 'wp_enqueue_scripts', 'chromaticfw_register_scripts', 0 );

/* Load Core scripts. */
add_action( 'wp_enqueue_scripts', 'chromaticfw_enqueue_scripts', 5 );
 ...
 * @return void
 */
function chromaticfw_register_scripts() {

	/* Supported JavaScript. */
	$supports = get_theme_support( 'chromaticfw-core-scripts' );
	...
	if ( isset( $supports[0] ) && in_array( 'mobile-toggle', $supports[0] ) )
		wp_register_script( 'mobile-toggle', esc_url( trailingslashit( CHROMATICFW_JS ) . "mobile-toggle{$suffix}.js" ), array( 'jquery' ), '20130528', true );
}
 ...
 * @return void
 */
function chromaticfw_enqueue_scripts() {
