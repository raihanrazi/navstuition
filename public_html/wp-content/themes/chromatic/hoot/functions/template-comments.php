 ...
 * @return void
 */
function chromaticfw_comment_reply_link( $args = array() ) {
	echo chromaticfw_get_comment_reply_link( $args );
}
 ...
 * @return string
 */
function chromaticfw_get_comment_reply_link( $args = array() ) {
 ...
 * @return array
 */
function chromaticfw_list_comments_args( $args = array() ) {
	...
	return apply_filters( 'chromaticfw_list_comments_args', wp_parse_args( $args, $defaults ) );
}
 ...
 * comment.php.
 *
 * The templates are saved in $chromaticfw->comment_template[$comment_type], so each comment template
 ...
 * @return void
 */
function chromaticfw_comments_callback( $comment, $args, $depth ) {
	global $chromaticfw;
	...
	if ( !isset( $chromaticfw->comment_template) || !is_array( $chromaticfw->comment_template ) )
		$chromaticfw->comment_template = array();

	/* Check if a template has been provided for the specific comment type.  If not, get the template. */
	if ( !isset( $chromaticfw->comment_template[$comment_type] ) ) {
		...
		$templates = apply_filters( 'chromaticfw_comment_template_hierarchy', $templates, $comment_type );

		/* Locate the comment template. */
		$template = locate_template( $templates );

		/* Set the template in the comment template array. */
		$chromaticfw->comment_template[ $comment_type ] = $template;
	}

	/* If a template was found, load the template. */
	if ( !empty( $chromaticfw->comment_template[ $comment_type ] ) )
		require( $chromaticfw->comment_template[ $comment_type ] );
}

/**
 * Ends the display of individual comments. Uses the callback parameter for wp_list_comments(). 
 * Needs to be used in conjunction with chromaticfw_comments_callback(). Not needed but used just in 
 ...
 * @return void
 */
function chromaticfw_comments_end_callback() {
	echo '</li><!-- .comment -->';
}
