 ...
 * @return string
 */
function chromaticfw_get_content_template() {
	...
	$templates = apply_filters( 'chromaticfw_content_template_hierarchy', $templates );

	/* Apply filters and return the found content template. */
	include( apply_filters( 'chromaticfw_content_template', locate_template( $templates, false, false ) ) );
}
 ...
 * @return void
 */
function chromaticfw_get_menu( $name = '' ) {
 ...
 * @return void
 */
function chromaticfw_get_header( $name = null ) {
 ...
 * @return void
 */
function chromaticfw_get_footer( $name = null ) {
 ...
 * @return void
 */
function chromaticfw_get_sidebar( $name = null ) {
 ...
 * @return void
 */
function chromaticfw_get_widget( $name = '' ) {
	include( chromaticfw_locate_widget( $name ) );
}
 ...
 * @return void
 */
function chromaticfw_locate_widget( $name = '' ) {
	$templates = array();
...
/**
 * A function for locating a shortcode template.  This works similar to the WordPress `get_*()` template functions. 
 * It's purpose is for loading a shortcode display template.  This function looks for shortcode templates within the 
 * hoot-theme sub-folder or the chromaticfw framework folder.
 *
 * @since 1.1.0
 * @access public
 * @param string  $name
 * @return void
 */
function chromaticfw_locate_shortcode( $name = '' ) {
	$templates = array();

	if ( '' !== $name ) {
		$base = str_replace( trailingslashit( THEME_DIR ), '', trailingslashit( CHROMATICFW_THEMEDIR ) );
		$templates[] = $base . "shortcodes/{$name}.php";
		$base = str_replace( trailingslashit( THEME_DIR ), '', trailingslashit( CHROMATICFW_DIR ) );
		$templates[] = $base . "extensions/shortcodes/display/{$name}.php";
		return locate_template( $templates, false );
	}
